 ...
 * 卡钞
 */
public class Wedge_Activity extends BaseActivity implements OnClickListener
{
    ...
    /*private AddMoneyAtmAtmVo_Dao atm_dao;
    private AddMoneyBranchInfoVo_Dao dot_dao;
    private AtmCheckVo_Dao check_dao;*/
...
//        atm_dao=new AddMoneyAtmAtmVo_Dao(db);
//        dot_dao=new AddMoneyBranchInfoVo_Dao(db);
//        check_dao=new AtmCheckVo_Dao(db);
    ...
    public void onClick(View arg0)
    {
        ...
        {
//            if(getResources().getString(R.string.add_wedge_dialog_choose_locother).equals(picker_loc.getresult()))
//            {
//                    //其他
...
//            else
    ...
    private void setDataToDb(MyAtmError error, int witch) {
        ...
        error.setItemtype("2");
        error.setIsback("Y");//是否带回
        if (tv_loc.isShown())
            error.setLocation(tv_loc.getText().toString());
        else
    ...
    private void showCardDialog() {
        ...
        View view = LayoutInflater.from(this).inflate(R.layout.dialog_card_input, null);
        ...
        bt_ok.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View arg0)
            {
				...
				if (cardInput == null || "".equals(cardInput)) {
                    CustomToast.getInstance().showLongToast(getResources().getString(R.string.input_card_tip));
				} else {
					int length = cardInput.length();
					if (length == 1) {
						if (cardInput.equals(secondConfirm)) {
							showDialog();
							edt_code.setText("000003000" + cardInput);
						} else {
						...
						if (cardInput.equals(secondConfirm)) {
							showDialog();
							edt_code.setText("00000300" + cardInput);
						} else {
                            CustomToast.getInstance().showLongToast(getResources().getString(R.string.input_error_tip));
						}
					} else if (length == 3) {
						if (cardInput.equals(secondConfirm)) {
							showDialog();
							edt_code.setText("0000030" + cardInput);
						} else {
                            CustomToast.getInstance().showLongToast(getResources().getString(R.string.input_error_tip));
						}
					} else if (length == 4) {
						if (cardInput.equals(secondConfirm)) {
							showDialog();
							edt_code.setText("000003" + cardInput);
						} else {
                            CustomToast.getInstance().showLongToast(getResources().getString(R.string.input_error_tip));
						}
					}
...
//            where_error.put("itemtype", "4");
            ...
            if(errors!=null&&errors.size()>0){
            	...
            	if(Util.isKaChao(scanResult)){
            		showDialog();//计时完毕时触发
            	}else{
            		CustomToast.getInstance().showToast(Wedge_Activity.this, getResources().getString(R.string.card_barcode_ok), 3000);
            	}
        }
        @Override
            ...
            bt_wedge_del.setOnClickListener(new OnClickListener()
            {
                ...
                public void onClick(View arg0)
                {
                    ...
                    if(adapter.getCount()==0)
                    {
                      ...
                      /*//atm检查信息也要修改状态
                        Map<String, Object> where_check=new HashMap<String, Object>();
        	   	   	   	 where_check.put("clientid", clientid);
        	   	   	   	 where_check.put("atmid", atm_bean.getAtmno());
        	   	   	   	 List<AtmCheckVo> checks=check_dao.quaryForDetail(where_check);
        	   	   	   	 if(checks!=null&&checks.size()>0)
        	   	   	   	 {
        			   	   	   AtmCheckVo check=checks.get(0);
        				   	   	check.setIsstuckmoney("N");
        				   	   	try {
        							check_dao.update(check);
